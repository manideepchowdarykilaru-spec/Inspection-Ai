...
| `npm run extract:test` | Run the field extractor over the transcribed carton fixtures in `backend/scripts/fixtures` and report every mismatch |
| `npm run nlp:train` | Regenerate the corpus (plus the real OCR lines in `dataset/lines.jsonl`) and retrain the declaration-section classifier (writes `backend/src/ocr/nlp/model.json`) |
| `npm run dataset:template` | Run the recogniser over the photographs in `dataset/images/` and write a label sheet pre-filled with its guesses |
| `npm run dataset:eval` | Score the recogniser against `dataset/labels.csv`; writes `dataset/report.md` (per-field accuracy and every failure) and `dataset/lines.jsonl` |
...
- **Forgot password** (`/forgot-password`) works in two steps: identify the account by Official ID or e-mail, then enter the 6-digit one-time code and a new password. Codes are hashed like passwords, expire after ten minutes and are voided after five wrong attempts. The code is delivered by e-mail through any SMTP account (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`; a Gmail App Password works) and by SMS through Twilio (`TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_FROM`; a trial account can text verified numbers). With neither configured the API returns the code for on-screen display, clearly labelled as demo mode, and logs it on the server; set `OTP_SHOW_ON_SCREEN=false` in production so a reset is refused instead when no channel could deliver.
...
**Licence numbers follow the product.** Food packs carry a 14-digit FSSAI licence; drugs, cosmetics and ayurvedic products carry a manufacturing licence ("Mfg. Lic. No.: UK.AY-191/2010") instead, so the licence declaration accepts either, and on a multi-unit carton picks the licence of the unit the batch-code prefix names. An importer declaration is required only for imported goods, so on a pack that declares "Made in India" it is reported as *not applicable* with the reason rather than as missing, and the declaration count is shown against the applicable total.

**A photograph without a readable label produces no inspection.** If fewer than six confident real words and fewer than two declarations come out of every pass, the scan is reported as *no label detected*: every declaration is returned empty, the workspace shows a re-capture prompt instead of a score, and no record with a nonsense product name can be saved. This was added after the first live scans turned out to be webcam frames with no package in them.
...
| Busy background (a room, a hand, shelves) | **Word-density crop** | The orientation probe already reads the frame at 900 px; the cluster of confidently read words, outliers dropped, locates the label when ink density cannot, because clutter binarises to ink but not to words. The probe also yields the tilt of the text lines (median of per-line slopes, which survives perspective fan) and the median print height, which drive the deskew and the upscale |
| Dot-matrix price, batch and dates | **Dot-matrix mode** | The median filter that removes speckle also removes the dots of dot-matrix glyphs, so the second-orientation pass that reads that block uses a light blur that joins the dots into strokes instead |
...
When the image had to be turned to read the body text, the untouched orientation is read once more and merged: on many cartons the dot-matrix price, batch code and dates are printed upright while the captions run sideways, and each orientation is read where it is legible. A lone alphanumeric code such as "RU3743 L8B" is accepted as the batch number at reduced confidence when no caption links to it. **A text classifier tags every line with its declaration section.** Alongside the caption patterns, a softmax model over hashed word, bigram, character-trigram and shape features (currency mark, date, PIN code, phone, unit, licence-length number, corporate suffix, address words…) classifies each OCR line as one of the twelve declarations or *other* (ingredients, directions, marketing). It is trained by `npm run nlp:train` on a generated corpus of 12,600 Indian packaging phrasings with OCR-style noise, reaches 99% on a held-out split, ships as a 70 KB int8 model inside the API and runs in under a millisecond per line. Where a caption is missing or garbled the tag says what a line is and the class's own pattern pulls the value out — an amount for MRP, a date for expiry, fourteen digits for a licence — so a mis-tag cannot inject nonsense. Text Extraction shows the tag under each line.
...
### Label classes — cardboard cartons first

Packaging falls into a few physical classes — cardboard cartons, flexible packets and pouches, curved bottles and jars, tins — and each puts the declarations in predictable places. The pipeline is being tuned class by class from photographs of real packs, cardboard first. Twelve toothpaste and soap cartons (Himalaya, Mamaearth, Pepsodent, Colgate, Medimix, Dove, Sensodyne, Dabur Red, Closeup, and a Pepsodent twin pack: six back panels, five long side panels and one legend-keyed panel) share one layout family:

| Detail | Where it sits | Phrasing seen |
|---|---|---|
| Brand and product name | Top-left, largest type | "Himalaya · Complete Care Herbal Toothpaste" |
| Ingredients, directions, warnings | Upper half, running text | "Ingredients: …", "For external use only." |
| Manufacturer and address | Middle-left block, two to four lines | "Manufactured by:", "Mfd. by:", or a bare "Unilever Limited" |
| Importer | Under the manufacturer when foreign-made | "Imported & Marketed by: Hindustan Unilever Ltd., …" |
| Licence number | Directly under the address | "Lic. No.: AUS-782", "Lic. No.: 106-ISM(HR)" |
| Country of origin | "Made in India." near the address, or implied by the address country only | Pepsodent: Indonesia, implied |
| Net quantity | Right side; sometimes "Net Wt. When Packed" with the value on the next line | "Net Wt. 100 g", "125 g" |
| MRP, batch, Mfg. date, Use Before | A bordered four-row box bottom-left, dot-matrix values; or a legend ("*MRP, #MFD, @Expiry") with one symbol-keyed dot-matrix line ("B1 *₹150 #02/22 @01/24") | "MRP ₹ (incl. of all taxes): ₹ 60.00", "Use Before: 11/2027" |
| Barcode | Bottom-right beside the box | EAN-13 |
| Consumer care | On the long side panel, between the address block and the MRP box | "For customer care: 1800 102 2202", "Lever Care: 1800-10-22-221 (Toll Free)", "For consumer complaints: … care@unilever.com" |

Their transcribed panels are the extraction fixtures (`npm run extract:test`, 157 expectations across 24 cartons), and their phrasing is part of the classifier's training corpus. What this class taught the extractor: "Use Before" as an expiry caption; plain "Lic. No." licences with bracketed codes; "Net Wt. When Packed" with the value on the following line; MRP lines where the rupee sign is misread; the country of origin implied by the manufacturer's address (which is also what makes Pepsodent read as imported and its importer block required); address blocks that stop at licence lines and pack claims; and a product name that is never an ingredient list or a marketing sentence.

### Training on real photographs — pouches, bottles, tins

Cartons were the first class; the second round of training used 44 shop-floor photographs taken on a phone (flexible pouches and wrappers, PET and glass bottles, a tin, a can, weigh-price stickers, many rotated 90° or 180°, all with shelf glare). They live in `dataset/images/` with hand-transcribed ground truth in `dataset/labels.csv`, and `npm run dataset:eval` scores the recogniser against them field by field, writing `dataset/report.md` with every failure. That report is the training loop: read the failures, fix the preprocessing or the extractor, run again. The same run exports every OCR line with the declaration it carries to `dataset/lines.jsonl`, which `npm run nlp:train` folds into the classifier corpus, so the tagger learns the real recogniser output for these packs rather than only synthetic templates. `dataset/README.md` documents the workflow and the label classes.

What this round taught the pipeline:

- **Light print on a dark label** (Coca-Cola, Bisleri, Nyle, Savlon, Sunfeast) was never inverted, because polarity was judged on the whole frame and the shelf around the bottle is bright. When a read comes back thin, the opposite polarity is tried; the words it finds outline the label, which is then read on its own at full resolution.
- **Legend blocks** — captions in one column ("MRP ₹ / Lot No.: / MFD.: / Use by:"), inkjet values in another, or a table header with a values row under it — come out of the recogniser as separate lines. A caption-column stage joins them: first by row, then by type over everything printed inside the block's reach (amounts to MRP, the earliest date to packing and the latest to expiry, an alphanumeric code to batch).
- **Small print drives the upscale.** The probe's median word height was dominated by the brand and the net quantity, so the legal block stayed at ten pixels. The 30th percentile of word heights now sets the scale, within the same output ceiling.
- **Month names** ("02JUN2026", "MAY/2026", "July 2026") are dates; "(1.17%)" is not.
- **Hallucinations were cut** at the source: a misread rupee mark ("?", "=") is believed only with paise or an MRP caption on the panel; a lone digit after ₹ is a fragment; a name, address or manufacturer that does not read as words is dropped; a contact block without a phone, e-mail or web address is not consumer care; packing after expiry means the legend rows were shifted and the two are swapped.
- **A wall-clock budget** (45 s by default, `OCR_BUDGET_MS`) bounds the optional passes so a difficult photograph degrades to fewer reads rather than a stalled request.

Result on the 47-image set with the local engine: 251 of 564 field checks correct (44.5%), up from 224 (39.7%) before this round, with hallucinated values cut from 22 to 15. With a hosted recogniser (OCR.space, see below) the same photographs score 384 of 564 (68.1%): net quantity 80%, manufacturer 76%, country 71%, consumer care 71%, packing date 64%, expiry 64%, MRP 58%, licence 57% of the printed values found, and no hallucinated addresses, contacts or licences. The remaining misses are overwhelmingly recogniser-level — text the OCR engine never read (curved bottles, sub-10-pixel print, glare) — which is why the demonstration flow asks the officer to photograph the panel square-on and close, and to add the neck or lid as a second image when a bottle says "see neck".
...
**Text Extraction** (sidebar) reads every printed line from a photograph and shows the original and the preprocessed image side by side, with each line's confidence and a slider to filter by it; hovering a line highlights where it came from on the original. Beside the text, a **Mandatory declarations** panel lists all twelve declarations — product identity, manufacturer and address, net quantity, MRP, consumer care, packing and expiry dates, country of origin, importer, FSSAI licence and batch — with the value read for each, its confidence, and a click that outlines where on the photograph it was read; declarations not found on that image are shown as missing so the gaps are visible before the rule engine runs. The scan workspace has the same view behind **What the OCR saw**. Both list the exact stages that were applied — including the detected tilt and whether the label was refocused.
...
### A stronger recogniser for phone photographs

The local engine (tesseract.js) is bundled so the prototype runs anywhere with no account and no cost, and it reads clean, upright cartons well. On phone photographs of curved bottles, glossy pouches and sub-ten-pixel print it reads little — which is the ceiling on the real-photo score above. The pipeline therefore accepts a hosted recogniser as a drop-in: set `OCR_PROVIDER=google` with a `GOOGLE_VISION_API_KEY` (Cloud Vision, document text detection; 1,000 images a month free) or `OCR_PROVIDER=ocrspace` with an `OCRSPACE_API_KEY` (free key by e-mail, no card). The provider returns words with boxes; everything after that — the column split, the caption readers, the legend stage, the classifier, the evidence boxes, the rule engine — is the same code that runs on the local engine's output, so nothing about the compliance logic changes. The local engine remains the fallback whenever the provider is unreachable or returns too little to be a label, and the workspace shows which engine produced the read.
